...
function neat_slider( $atts ) {
	...
	while ( have_posts() ) : the_post();
 	  ...
 	  $neat_slides = get_post_meta($id,'neat_slides');
	  $slider_settings = get_post_meta($id,'slider_settings'); 
	...
	foreach ($neat_slides[0] as $key => $value) {  
	  ...
	  $list .= '<script type="text/javascript">
	  				 jQuery(window).load(function(){
	  				 	jQuery(".neat-slider-'. $sliderid .'").fractionSlider({
	  				 		"fullWidth": '. $fullWidth .',
				...
				$list .= ' <script type="text/javascript">
	  			jQuery(window).load(function(){ 
	  				...
	  				if (viewportWidth > 1200) {
	  					...
	  					jQuery(".slider-wrapper").css({"width": viewportWidth,"max-width": viewportWidth,"margin-left":"-"+marginslidebg+"px", "max-height": "'. $slider_height .'px",});
	  				} else
	  				{
