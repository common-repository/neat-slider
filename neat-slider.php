<?php 
/*
Plugin Name: Neat Slider
Plugin URI:  
Description: Neat Slider is powerful and easy to use WordPress responsive slider plugin.
Author: Amjad Ali
Version: 1.2
...
function add_content_after_editor() {
     ...
     global $post_type;
    if( 'neat-slider' == $post_type )  {

    global $post;
    $id = $post->ID;  

	echo '<div class="postbox"><div class="inside">';
        echo '<p> <span style="margin-right:10px; font-weight:bold;">Shortcode:</span> <input type="text"  value="[neat-slider id=&quot;'.  $id .'&quot;]" readonly="" style="padding: 4px;text-align: center; font-size: 14px;" >  </p>';
	echo '</div></div>';
...
function setup_slider_fields() {
	if ( defined( 'FM_VERSION' ) ) {
		$dir = dirname( __FILE__ ) . '/lib/';

		# Supporting code
		require_once( $dir . 'class-slider-data-structures.php' );
   
		require_once( $dir . 'class-slider-fields.php' ); 
 
	} else {
		# Add an error message saying, "hey, something worng"
	}
}
add_action( 'after_setup_theme', 'setup_slider_fields' );

 
add_action('init','slide_js_init');

/************* Enqueue css files *********************/  

function neat_slider_css() {
  if ( !is_admin() ) {
  
     wp_register_style( 'neat_slider_style',plugins_url( '/css/slider-style.css', __FILE__ ), false );
     wp_enqueue_style( 'neat_slider_style' ); 


  }
}  
add_action( 'init', 'neat_slider_css' );

function slide_js_init() {

    wp_register_script( 'slider-js',  plugins_url( '/js/jquery.fractionslider.min.js', __FILE__ ), array( 'jquery' ), false, true );
  	wp_enqueue_script( 'slider-js' );

}

function neat_slider_template($single_template) {
     global $post;

     if ($post->post_type == 'neat-slider') {
          $single_template = dirname( __FILE__ ) . '/lib/neat-slider-template.php';
     }
     return $single_template;
}
add_filter( 'single_template', 'neat_slider_template' ); 